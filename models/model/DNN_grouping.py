...
class DNN:
    ...
    def group(self, train) :
        train['creator_account_creation_group'] = train['creator_account_creation'].apply(grouping, args=(1136041200, 31536000))
        train['creator_follower_count_group'] = train['creator_follower_count'].apply(grouping, args=(0, 0, [10, 100, 1000, 100000]))
        train = train.drop('creator_account_creation', axis=1)
        train = train.drop('creator_follower_count', axis=1)
        return train
    ...
    def train(self, TARGET_id=3):
        ...
        for i, train in tqdm(enumerate(self.df)):
            ...
            model.fit(x = X_train,
                          ...
                          batch_size=32) 
            #save model
            model_path = f'/hdd/cpu_models_DNN_grouping/model-{TARGET}-{i}'
    ...
    def predict(self, TARGET_id=3):
        ...
        model = joblib.load( f'/hdd/cpu_models_DNN_grouping/model-{TARGET}-288' )
